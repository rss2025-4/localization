...
class SensorModel:
    ...
    def evaluate(self, particles, observation):
        ...
        # particle_probabilities = np.zeros((len(particles), 1))
        ...
        # particle_probabilities = np.array(np.power(particle_probabilities, 1/2.2))
        log_sum = np.sum(np.log(p_z), axis=1)
        particle_probabilities = np.exp(log_sum - log_sum.mean())
